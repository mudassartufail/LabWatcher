...
$objUser = new User();
$objHardware=new errors();
$objUser->isLogin();

startHtml('Hardware Errors');
topHeader();
leftNavigation();
?>
...
{
	list($do, $hardId) = explode('|', $objUser->cleanString($objUser->decrypt($_REQUEST['do'])));
	$objHardware->id = $objUser->cleanString($hardId);
	$objHardware->deleteError();
	
	//$deleted = $objHardware->wasDeleted();
...
function confirmBox(param)
{
	...
	$( "#dialog-confirm" ).dialog({
		...
		buttons: {
			"Delete": function() {
				$( this ).dialog( "close" );
				window.location = 'view_hardware_errors.php?do='+param;
			},
			Cancel: function() {
				$( this ).dialog( "close" );
			}
            ...
            <div class="widget" style="width:90% !important">
                ...
                <div class="block-fluid">
                    ...
                    <table class="fpTable table-hover" cellpadding="0" cellspacing="0" width="100%">
                        ...
                        <tbody>
                        <?php
                            $hardwareErrors=$objHardware->getHardwareErrors();
						if($hardwareErrors)
						{	
                            $count=0;
                            foreach($hardwareErrors as $error)
                            {
                                $count +=1;
								$doView=$error->Id;
